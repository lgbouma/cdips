...
\begin{figure*}[!t]
	...
	\caption{
		...
		TOI~496, in NGC 2548;
		TOI~625;
		an eccentric EB, masquerading as a planet.
		{\it Middle row, left to right}:
		A detached EB in a 80~Myr old cluster;
		V684 Mon (a 10 Myr old detached EB);
		a semi-detached EB in NGC 2548.
		{\it Bottom row, left to right}:
		Spotted rotator in NGC 2184;
		Pulsator or perhaps EB, flagged by \citet{zari_3d_2018} as a pre-MS star;
		Quasi-periodic pulsator, also with a pre-MS host.
		Gaia source identifiers are given in a footnote.
	\label{fig:quilt}
	}
\end{figure*}
...
A detailed search for transiting planets will be the subject of future
work.
The light curves provide opportunities for studying many other types of
variables, including
pulsating stars, rotators, and eclipsing binaries.
A few hand-picked examples are shown in Figure~\ref{fig:quilt}.
The soure identifiers are in a footnote\footnote{
