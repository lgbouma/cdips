...
\begin{rotatetable*}
\begin{deluxetable*}
    \tablecaption{CDIPS target stars collected for the present work.  
    ...
    \tablehead{
      ...
      \colhead{$G_{\rm Bp}$} &
      \colhead{$G_{\rm Rp}$} &
      \colhead{K13 name match} &
      \colhead{Unique cluster name} &
      \colhead{How match} &
      \colhead{Not in K13?} &
      \colhead{Comment} &
      \colhead{K13 logt} &
      \colhead{K13 err logt}
    }
    ...
    \tablecomments{
        Table~\ref{tbl:cdips_targets} is published in its entirety in a 
