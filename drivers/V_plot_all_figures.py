...
def plot_target_star_hist_logt(OC_MG_CAT_ver=None, overwrite=1):
    ...
    n_total = len(mdf)
    mdf = mdf[~pd.isnull(mdf['mean_age'])]
    n_with_age = len(mdf)

    f,ax = plt.subplots(figsize=(4,3))

    bins = np.arange(-0.5,11.0,0.5)
    ax.hist(mdf['mean_age'], bins=bins, cumulative=False, color='black',
...
def plot_hrd_scat(sectors, overwrite=0, close_subset=0, use_actual_lcs=0,
                  closest_subset=0):
    ...
    if close_subset:
        ...
        outpath = os.path.join(OUTDIR, prestr+'hrd_scat_closest_subset.png')
    else:
    ...
    plxkey = 'parallax' if not use_actual_lcs else 'Parallax[mas]'
    M_omega = df['phot_g_mean_mag'] + 5*np.log10(df[plxkey]/1000) + 5
    ...
    if close_subset:
        ...
        txtstr= (
            '$\omega>0$, $1/\omega_{\mathrm{as}} < 300$\n'+'{} stars'.
            format(len(df))
        )
    else:
        txtstr= (
