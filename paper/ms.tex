...
\title{
  ...
  I. Methods \& Light Curves from TESS Sectors 1-5
}
...
(\S~\ref{subsec:lcdetrending}), and then assess the resulting
lightcurves using a variety of statistics for their white noise and
red noise properties (\S~\ref{sec:results}).
...
Second, we estimate the background of each frame.  We do this by
...
The typical size of scattered light patches is greater than 32 pixels,
...
Finally, for frames with the \texttt{DQUALITY} bit-flag set to
32~---~corresponding to the ``momentum dumps'' described by
\citet{vanderspek_2018}~---~we mask out the entire frame.  This
removes on average a few frames per sector. Through visual inspection,
we see that the stars on these frames are extremely smeared, and are
unlikely to produce useful science data.
