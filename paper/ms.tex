...
(independent from the solution included with the frames), we use
...
onto each frame.
Initially, we intended to photometer all Gaia-DR2 sources in each field
down to a cutoff of $G_{Rp} < 16$.
However, we found that for the galactic plane fields this produces an
excessively large number of sources.
Our source catalog for each frame is therefore a combination of
the CDIPS target stars ($G_{Rp} < 16$), and all Gaia-DR2 sources down to
$G_{Rp} < 13$.
We use the expected positions of these sources
...
preferable to performing source extraction simply because the TESS
pixels are large, and we are operating in crowded fields.
The Gaia-DR2 epoch is
...
of their predicted positions in the TESS images.  The projection from
catalog sky-coordinate positions to pixel coordinates
is performed using standard WCS transformations, 
and the source catalog look-up is
performed using
...
Equation~\ref{eq:ism_flux_measurement}. To finally convert from a list of 
...
\begin{figure*}[t]
    \begin{center}
		\includegraphics[height=0.9\textheight]{f4.png}
    \end{center}
    \vspace{-0.5cm}
    \caption{
