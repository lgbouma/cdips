...
% BEGIN NUMBERS
\newcommand{\numberpcs}{29\ }  % 2019/06/28 unanimous gold
...
\citet{Kharchenko_et_al_2013} whenever possible, following the
procedure described in Appendix~\ref{appendix:uniquenames}.  For
moving groups not identified in \citet{Kharchenko_et_al_2013}, we
adopted the constellation-based naming convention from
\citet{gagne_banyan_XI_2018}.  Otherwise, we used the name reported by
the original catalog.  This process reduced 16,425 name permutations
down to 3,216 unique cluster names.  Though we have made every effort
to avoid duplicates, a small number may remain in this column, so we
advise inspection of the \texttt{cluster} and \texttt{reference}
columns rather than the \texttt{unique_cluster_name} column when
analyzing individual objects of interest.  Nonetheless, 87.7\% of the
$\sim$million unique target stars are matched to clusters within
...
\begin{figure}[!t]
	%\gridline{
	%  \fig{cluster_field_star_positions.png}{0.49\textwidth}{}
	%  \fig{sector6_cam[1]_ccd[1-2-3-4]cluster_field_star_positions.png}{0.49\textwidth}{}
	%}
	\begin{center}
		\leavevmode
...
\begin{figure}[!t]
    \gridline{\fig{tlsresult_sector6.png}{0.45\textwidth}{}}
    \vspace{-0.8cm}
    \gridline{\fig{tlsresult_sector7.png}{0.45\textwidth}{}}
    \caption{
...
\begin{figure*}[t]
	\begin{center}
		\leavevmode
		\includegraphics[width=0.9\textwidth]{ctoi_quilt.png}
	\end{center}
	\vspace{-0.2cm}
	\caption{
    {\bf 12 of the \numberpcs candidate transiting planets identified
    in this work.} 
    Models are fit as described in \S~\ref{subsec:ctois}.
    The full set of model parameters are in Table~XXX and on
    exofop-TESS.
...
Finally, the determine the names of the objects -- and whether they
...
\begin{enumerate}[(i)]
  ...
  \item checked whether the SIMBAD online name resolving service had
...
A few edge-cases, including for instance sub-clusters within larger
star-forming complexes like in Sco-Cen or Collinder~33, were 
manually resolved to the extent feasible \citep[][give
detailed pictures of the complex morphologies that frequently arise in
very young star-forming
regions]{rizzuto_multidimensional_2011,saurin_isolating_2015}.
...
\begin{figure*}[!h]
	...
	\caption{
		...
		under the assumption that the star is a dwarf.%\footnote{\url{http://www.pas.rochester.edu/~emamajek/EEM_dwarf_UBVIJHK_colors_Teff.txt}}.
		\label{fig:pg3}
	}
\end{figure*}
