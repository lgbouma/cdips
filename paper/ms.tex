...
\shorttitle{CDIPS I: Method, Sectors 6 \& 7}
...
\begin{abstract}
  ...
  photometrically monitor almost every nearby star cluster in
  the Galaxy.
  % Aims
  As part of the CDIPS project, we have begun processing the TESS
  full-frame images to make high-precision light curves for candidate
  members of stellar associations, moving groups, and open clusters.
  %
  ...
  Each light curve repesents between 20 and 25 days of observations
  of a star brighter than $G_{Rp}=16$, sampled at 30 minute cadence.
  % Methods
  We describe in detail the image subtraction and light curve
  processing techniques we used to create the light curves, which
  have noise properties in-line with theoretical expectations.
  % Results
  We then highlight the \numberpcs planet candidates identified from
  an initial search, describe the vetting procedures that they have
  survived, and emphasize the need for additional
  ...
  damping.
  %
  The data products include both the light curves and also vetting
  reports, all of which are available at
...
\begin{figure}[t]
    \begin{center}
        \leavevmode
        \includegraphics[width=0.49\textwidth]{sector6_cluster_field_star_positions.png}
    \end{center}
    \vspace{-0.5cm}
    \caption{
        {\bf Celestial positions of light curves from Sector 6}
...
\begin{figure}[t]
  ...
  calculated using the updated figures from \citet{vanderspek_2018}.
