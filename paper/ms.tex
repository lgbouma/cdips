...
\begin{figure}[t]
    ...
    \gridline{\fig{target_star_cumulative_counts.png}{0.4\textwidth}{}}
    \vspace{-0.8cm}
    \gridline{\fig{target_star_hist_logt.png}{0.4\textwidth}{}}
    \vspace{-0.8cm}
    \gridline{\fig{target_star_reference_pie_chart.png}{0.4\textwidth}{}}
	\end{center}
	\vspace{-0.8cm}
	\caption{
    {\it Top.} Cumulative counts of target stars as
    a function of Gaia $Rp$-band magnitude.  The stars are
...
$\Delta x = x-x_0$, $\Delta y = y - y_0$, and $A$ and $B$ arbitrary
flux-scaling constants.
...
\begin{figure}[!t]
	...
	\vspace{-0.5cm}
  \caption{
    {\bf Percentiles of the average autocorrelation of
    $10^4$ randomly selected light curves from each sector.} The
    25$^{\rm th}$, 50$^{\rm th}$, and 75$^{\rm th}$ percentiles at
    time lags spaced by one hour are
    shown by dashed, solid, and dotted lines respectively.  The raw
    light curves (column name \texttt{IRM}) are shown in gray, and the
...
The preceding discussion is only relevant to the white noise
...
\begin{figure}[!t]
	...
	\vspace{-0.8cm}
	\caption{
    (IN PROGRESS) Significance thresholds empirically defined for the
    transiting planet search described in
    \S~\ref{subsec:identifying_ctois}.  Each point represents the TLS
    periodogram peak signal detection efficiency from one light curve.
...
\end{enumerate}
The initial classifications were performed by LGB, with possible
planet candidates inspected by JH and JNW.  The \texttt{TagSpaces}
...
Maybe JH or JNW could contribute these subsections?
