...
\begin{abstract}
  ...
  full-frame images to make light curves of stars that
...
However, since most dwarf star properties do not appreciably change
until the stars evolve off the main sequence, creative approaches must
be used to infer stellar ages \citep{soderblom_ages_2010}.
...
analyses, and are typically most constrained by the location of the
...
the first hot Jupiters in a cluster \citep{Quinn_et_al_2012}, and also
the first multi-planet system \citep{Malavolta_et_al_2016}.
...
The transit method was slow to catch up, despite deep searches of
M~37 by \citet{hartman_MMT_IV_2009}, as well as
searches of nine young clusters in the solar neighborhood
by the Monitor project
\citep{aigrain_monitor_2007,irwin_monitordata_2007,miller_monitor_2008}.
While these studies led to valuable measurements of stellar
...
Kepler \citep{borucki_kepler_2010} was the first project to observe a
...
and 67b, in the gigayear-old NGC~6811 \citep{Meibom_et_al_2013}.
...
K2-25b, discovered in the Hyades, had a radius ($3.43\,R_\oplus$) that was
larger than older Kepler planets at similar insolation levels
\citep{Mann_K2_25_2016}.  This inflated radius is perhaps a hint that the
planet is contracting, or losing its atmosphere (e.g., CITE).  The
super-Neptune sized K2-33b ($P\approx 5.4\,{\rm d}$), found in Upper Sco,
showed that at least some close-in planets must form within about 10 Myr,
excluding long-timescale migration as an explanation for its origin
\citep{Mann_K2_33b_2016,David_et_al_2017}.  Further discoveries included six
transiting planets in Praesepe \citep{obermeier_k2_2016,Mann_et_al_2017}, a
long-period candidate super-Earth in the Hyades
\citep{vanderburg_zeitVII_2018}, the three-planet K2-136 system in the Hyades
\citep{ciardi_k2-136_2018,livingston_three_2018,mann_ZEITVI_2018}, and also the
two-planet system K2-264 in Praesepe
\citep{rizzuto_zeitVIII_2018,livingston_k2-264_2019}.  A final recent
K2 detection is of a Jupiter-sized planet orbiting every 24 days around V1298
Tau \citep{david_v1298_2019}.
...
youth can be inferred based on a combination of rotation, activity,
and kinematic indicators \citep[{e.g.}, K2-233][]{david_k2-233_2018}.
To investigate demographics in time, the detection sensitivities and
reliabilities of these searchers need to be calibrated, likely with a
machinery similar to that outlined by \citet{rizzuto_zeitV_2017}.
A separate requirement for population studies to be statistically
...
For example, using Gaia-DR2 \citet{cantat-gaudin_newOCs_2019}
discovered a few tens of previously undetected open clusters in
the direction of Perseus. Earlier, \citet{cantat-gaudin_gaia_2018}
fortuitously discovered 60 open clusters as well; however a
homogeneous all-sky search with Gaia data has yet to be performed.


One of the major challenges in deriving precise photometry from the
TESS images is the relatively poor angular resolution ($\approx
21''$).  Almost all clusters are within 10 degrees of the Galactic
plane (see FIGURE~1). The problems with crowding and complex
backgrounds are so severe near the galactic plane that the {\it
TESS}\, Candidate Target List has deprioritized all objects at
galactic latitudes less than $\approx
15^\circ$~\citep{stassun_TIC_2018,stassun_TIC8_2019}, which includes
90\% of all star clusters\footnote{TICv8 updates this to $10^\circ$,
but for the first year of TESS observations, the number used when
selecting target stars was $15^\circ$.}.
This is sensible, given that the large pixel size and the high stellar
surface density will make aperture photometry unreliable.  However, it
also means that a large number of stars in clusters that could yield
planet candidates may go unprocessed by the official {\it TESS} data
reduction pipeline.

We have therefore opted to produce time-series photometry from the
TESS images for stars in ``clusters''~--~a collective term we adopt
for open clusters, moving groups, stellar associations, and
star-forming regions.
We are also including some stars that we suspect are young due to
combined photometric and astrometric indicators.
We are using the difference imaging technique because of its benefits
in modelling complex backgrouds in crowded regions.
Our eventual hope is to discover giant transiting planets, and probe
the exoplanet size and separation distributions for planets younger
than 1 Gyr.

%FIXME improve
In what follows, \S~\ref{sec:starselection} describes the process of
selecting stars.  \S~\ref{sec:method} presents the photometric and
image processing methods, and
\S~\ref{sec:results} describes results.
...
\begin{figure}[!ht]
	\gridline{\fig{pm_scat_all_CDIPS_LCs.png}{0.45\textwidth}{}}
	\vspace{-0.8cm}
	\gridline{\fig{pm_scat_close_subset.png}{0.45\textwidth}{}}
	\caption{
